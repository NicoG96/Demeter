...
class BitBucket():
    ...
    def cli(self):
        ...
        if len(issues) != 0:
            pull_requests, connect_errors = self.connect_pull_requests(self.get_pull_requests(), issues)
        ...
        prev_branch = self.get_remote_branch()
        ...
        new_branch = input()
        self.build_new_branch(prev_branch, new_branch)
        self.cherrypick(pull_requests, new_branch)
    ...
    def connect_pull_requests(self, all_pulls, issues):
    ...
    def build_new_branch(self, prev_branch, new_branch):
        self.repo.git.checkout(prev_branch)
    ...
    def cherrypick(self, pull_requests, new_branch):
        logging.info('Cherry-picking ' + str(len(pull_requests)) + ' commit' +
                    ('s' if len(pull_requests) > 1 else '') + '...')

        for pr in pull_requests:
            self.repo.git.cherry_pick('-m', '1', pr['merge_commit']['hash'])

        logging.info('Pushing changes to origin...')
        self.repo.git.push('--set-upstream', 'origin', str(new_branch))
    ...
    def execute(self):
        ...
        self.repo = Repo(config.get('LOCAL REPO', 'BITBUCKET_REPO'))
